...
def ebone_data_analyzer(file1, file2, label_name, config, max_tm_idx=30, save_plot=False):
    ...
    def pr_plot(file1, file2, scheme='mlu', label_name=None, config=None, save_plot=False):
        ...
        if scheme == 'mlu':
            plt.ylim(0.2, 1.005)
            # plt.title(r'Link Load Balancing Performance ($\mathrm{{PR_U}}$) among different rerouting schemes base on test traffic matrix (' + str(
            #         len(Y1)) + ' TMs)', fontsize=title_font_size)
            ...
            if save_plot:
                if config.method == 'actor_critic':
                    fig.savefig(
                        os.getcwd() + '/result/img/week_details_all-schemes-curve-ac-pr-mlu-week-' + label_name[
                            0] + '.png',
                        format='png')
                if config.method == 'pure_policy':
                    fig.savefig(
                        os.getcwd() + '/result/img/week_details_all-schemes-curve-pg-pr-mlu-week-' + label_name[
                            0] + '-' +
                        label_name[
                            1] + '.png',
                        format='png')
        ...
        if scheme == 'delay':
            ...
            # plt.title(r'End-to-End Delay Performance ($\mathrm{{PR_\Omega}}$) among different rerouting schemes base on test traffic matrix (' + str(len(Y1)) + ' TMs)',
            #     fontsize=title_font_size)
            ...
            if save_plot:
                if config.method == 'actor_critic':
                    fig.savefig(
                        os.getcwd() + '/result/img/week_details_all-schemes-curve-ac-pr-delay-week-' + label_name[
                            0] + '.png',
                        format='png')
                if config.method == 'pure_policy':
                    fig.savefig(
                        os.getcwd() + '/result/img/week_details_all-schemes-curve-pg-pr-delay-week-' + label_name[
                            0] + '-' +
                        label_name[
                            1] + '.png',
                        format='png')
...
if __name__ == '__main__':
    ...
    file1 = 'result/csv/result_pure_policy_conv_Ebone_alpha_update_lastK_centralized_sample_0.25scaleK_maxMoves10.csv'
    # file1 = 'result/csv/result_pure_policy_conv_Ebone_alpha_update_lastK_centralized_sample_0.25scaleK_maxMoves15.csv'
    file2 = 'result/csv/result_pure_policy_conv_Ebone_baseline_maxMoves10.csv'
    # file2 = 'result/csv/result_pure_policy_conv_Ebone_baseline_maxMoves15.csv'
    ...
    # label_name = ['CeFRO', 'CFR-RL', 'TopK Critical', 'TopK Cum-Centrality', 'TopK Centralized', 'TopK',
    #               'ECMP']
    label_name = ['PKE-DRL', 'CFR-RL', 'TopK Critical', 'TopK Cum-Centrality', 'TopK Centralized', 'TopK',
                  'ECMP']
    ...
    cdf_plot = False
    day_list = [1, 2, 3, 5]
    ebone_data_analyzer(file1, file2, label_name=label_name, config=config, save_plot=save_plot)
